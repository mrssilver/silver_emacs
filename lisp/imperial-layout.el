...
(with-selected-window  (get-window-with-purpose "I") (switch-to-buffer "*Completions*"))
(with-selected-window  (get-window-with-purpose "io") (switch-to-buffer "*erc*"))
(with-selected-window  (get-window-with-purpose "main") (switch-to-buffer "*scratch*"))
...
(defun force-display-buffer-window (orig-fun &rest args)
  "force display-buffer obey alist"
  (if split-window-enabled
      (apply orig-fun args)
    ;;alist-rule must be def
    (let ((display-buffer-alist '((alist-rule . ((display-buffer-reuse-window . t))))))
      (apply orig-fun args))
    )
  )


(advice-add 'display-buffer :around #'force-display-buffer-window)
;;force pop buffer obey rules
(defun force-switch-to-buffer (orig-fun &rest args)
  "force pop-to-buffer obey rules显示。"
  (if split-window-enabled
      (apply orig-fun args)
      ;;current window pop
...
(setq display-buffer-alist 'alist-rule)
;; 启动时确保布局存在
